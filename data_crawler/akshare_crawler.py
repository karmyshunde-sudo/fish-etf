...
def standardize_column_names(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 定义可能的列名变体
    column_variants = {
        '日期': ['date', '日期', 'trade_date', 'dt', 'datetime', '交易日期'],
        '开盘': ['open', '开盘价', '开', 'open_price'],
        '最高': ['high', '最高价', '高', 'high_price'],
        '最低': ['low', '最低价', '低', 'low_price'],
        '收盘': ['close', '收盘价', '收', 'close_price', 'price'],
        '成交量': ['volume', '成交量', 'vol', '成交数量', 'amount_volume'],
        '成交额': ['amount', '成交额', '成交金额', 'turnover', '成交总价'],
        '振幅': ['amplitude', '振幅%', '振幅百分比', 'amplitude_percent'],
        '涨跌幅': ['percent', '涨跌幅', '涨跌%', 'change_percent', 'pct_chg'],
        '涨跌额': ['change', '涨跌额', '价格变动', 'price_change'],
        '换手率': ['turnover_rate', '换手率', 'turnover_ratio', 'turnover']
    }
...
def ensure_required_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    确保DataFrame包含所有必需的列，缺失的列用默认值填充
    ...
    if missing_columns:
        logger.warning(f"⚠️ 数据源仍缺少必要列：{', '.join(missing_columns)}")
        ...
        for col in missing_columns:
            if col == '成交额':
                ...
                if '最高' in df.columns and '最低' in df.columns and '收盘' in df.columns:
                    ...
                    df['振幅'] = ((df['最高'] - df['最低']) / df['前收盘'] * 100).fillna(0)
                ...
                if '收盘' in df.columns:
                    df['前收盘'] = df['收盘'].shift(1)
                    df['涨跌幅'] = ((df['收盘'] - df['前收盘']) / df['前收盘'] * 100).fillna(0)
                    df = df.drop(columns=['前收盘'])
                else:
                    df['涨跌幅'] = 0.0
            elif col == '涨跌额':
...
def limit_to_one_year_data(df: pd.DataFrame, end_date: str) -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(f"限制数据为1年时发生错误: {str(e)}")
        return df
