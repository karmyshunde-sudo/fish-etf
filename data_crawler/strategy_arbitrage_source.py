...
# ===== 多数据源配置 =====
# 【关键修复】调整优先级：akshare（东方财富）提为第一优先级，因其有专门的IOPV实时估值字段
# 腾讯/新浪降级为备用数据源，因为它们的IOPV字段位置不确定（硬编码猜测）
SOURCE_PRIORITY = [
    (0, 0, 1),  # 数据源0-接口0：东方财富（akshare）- 最可靠的IOPV数据源
    (1, 0, 2),  # 数据源1-接口0：腾讯财经（备用，IOPV位置不确定）
    (2, 0, 3),  # 数据源2-接口0：新浪财经（备用，IOPV位置不确定）
...
def fetch_arbitrage_realtime_data() -> pd.DataFrame:
    ...
    try:
        ...
        # ===== 多数据源轮换逻辑 =====
        # 【关键修复】数据源顺序与SOURCE_PRIORITY保持一致
        DATA_SOURCES = [
            # 数据源0：东方财富（akshare）- 第一优先级，有专门的IOPV实时估值字段
            {
                "name": "东方财富",
                "interfaces": [
                    {
        ...
        for offset in range(total_priority):
            ...
            try:
                ...
                # 【关键修复】在标准化前，先进行IOPV数据交叉验证
                # 当出现极端溢价/折价时（|溢价率|>5%），用备用数据源验证真实性
                # 注意：不过滤极端数据，只验证数据真实性
                df = _validate_iopv_before_standardize(df, etf_codes, DATA_SOURCES, logger, interface["source_type"])
...
def _validate_iopv_before_standardize(df: pd.DataFrame, etf_codes: List[str], 
                                        data_sources: List[dict], logger, 
                                        primary_source_type: str) -> pd.DataFrame:
    """
    【关键修复】在标准化前验证IOPV数据真实性
    核心原则：不过滤极端异常数据（那可能是发大财的机会），而是用多数据源交叉验证
    
    验证逻辑（三源验证机制）：
    - 当|溢价率| > 5%时，认为是"潜在极端数据"，触发交叉验证
    - 先用备用数据源1验证主数据源
    - 如果两个数据源差异>20%，再用第三个数据源作为"裁判"来判定谁对谁错
    - 如果第三个数据源验证通过 → 数据真实，保留（可能是真实套利机会）
    - 如果三个数据源两两差异都很大 → 标记为"数据存疑"，但保留（可能是极端行情）
    ...
    # 确保数值类型
    df["市场价格"] = pd.to_numeric(df["市场价格"], errors='coerce')
    df["IOPV"] = pd.to_numeric(df["IOPV"], errors='coerce')
    
    # 计算初步溢价率
    valid_mask = (df["IOPV"] > 0) & (df["市场价格"] > 0)
    if not valid_mask.any():
        return df
    
    df.loc[valid_mask, "_temp_premium_rate"] = (
        (df.loc[valid_mask, "市场价格"] - df.loc[valid_mask, "IOPV"]) / 
        df.loc[valid_mask, "IOPV"] * 100
    )
    
    # 【关键】找出潜在极端数据：|溢价率| > 5%
    # 注意：不设置上限，48%溢价也可能是真实的（虽然概率低，但值得验证）
    extreme_mask = valid_mask & (df["_temp_premium_rate"].abs() > 5.0)
    
    if not extreme_mask.any():
        logger.info(f"IOPV验证：所有ETF溢价率均在5%以内，无需交叉验证")
        df = df.drop(columns=["_temp_premium_rate"])
        return df
    
    extreme_etfs = df.loc[extreme_mask, "ETF代码"].tolist()
    logger.warning(f"⚠️ IOPV验证：发现 {len(extreme_etfs)} 只ETF溢价率绝对值>5%，开始三源交叉验证")
    logger.warning(f"   极端ETF列表: {extreme_etfs}")
    
    # 确定备用数据源（排除主数据源）
    backup_fetch_funcs = []
    source_func_map = {
        "akshare": _fetch_akshare_etf_data,
        "tencent": _fetch_tencent_etf_data,
        "sina": _fetch_sina_etf_data,
        "yfinance": _fetch_yfinance_etf_data,
    }
    
    for source_name, fetch_func in source_func_map.items():
        if source_name != primary_source_type:
            backup_fetch_funcs.append((source_name, fetch_func))
    
    # 【三源验证机制】
    # 依次用备用数据源验证，当发现冲突时用第三个数据源作为裁判
    validated_count = 0
    corrected_count = 0
    still_uncertain_count = 0
    
    # 存储每个ETF的多源IOPV数据，用于三源验证
    etf_iopv_sources = {}  # {etf_code: [(source_name, iopv), ...]}
    
    for backup_name, backup_func in backup_fetch_funcs:
        if not extreme_etfs:
            break
        
        try:
            logger.info(f"   → 用 [{backup_name}] 验证 {len(extreme_etfs)} 只ETF的IOPV数据")
            backup_df = backup_func(extreme_etfs)
            
            if backup_df is None or backup_df.empty:
                logger.warning(f"   [{backup_name}] 返回空数据，跳过")
                continue
            ...
            if "ETF代码" not in backup_df.columns or "IOPV" not in backup_df.columns:
                continue
            backup_df["IOPV"] = pd.to_numeric(backup_df["IOPV"], errors='coerce')
            ...
            for idx in df[extreme_mask].index:
                ...
                # 记录该ETF的IOPV来源
                if etf_code not in etf_iopv_sources:
                    etf_iopv_sources[etf_code] = [(primary_source_type, df.loc[idx, "IOPV"])]
                
                etf_iopv_sources[etf_code].append((backup_name, backup_iopv))
                
        except Exception as e:
            logger.warning(f"   [{backup_name}] 验证过程出错: {str(e)}")
            continue
    
    # 【三源验证核心逻辑】对每个极端ETF进行多源对比
    for idx in df[extreme_mask].index:
        etf_code = df.loc[idx, "ETF代码"]
        ...
        sources = etf_iopv_sources[etf_code]
        primary_iopv = df.loc[idx, "IOPV"]
        primary_price = df.loc[idx, "市场价格"]
        primary_rate = df.loc[idx, "_temp_premium_rate"]
        
        # 如果只有一个数据源（主数据源），无法验证
        if len(sources) == 1:
            still_uncertain_count += 1
            logger.warning(f"   ❓ ETF {etf_code}: 只有主数据源，无法交叉验证")
            continue
        
        # 计算所有数据源之间的两两差异
        # 找出最接近主数据源的备用数据源
        primary_diffs = []
        for source_name, source_iopv in sources:
            if source_name == primary_source_type:
                continue
            diff_pct = abs(source_iopv - primary_iopv) / primary_iopv * 100
            primary_diffs.append((source_name, source_iopv, diff_pct))
        
        # 如果没有差异过大的数据源（所有差异<5%），验证通过
        all_close = all(diff_pct < 5 for _, _, diff_pct in primary_diffs)
        if all_close:
            validated_count += 1
            logger.info(f"   ✅ ETF {etf_code} IOPV验证通过: 多源数据一致")
            extreme_mask[idx] = False
            continue
        
        # 如果有差异过大的数据源，用第三个数据源作为裁判
        has_large_diff = any(diff_pct > 20 for _, _, diff_pct in primary_diffs)
        
        if has_large_diff and len(sources) >= 3:
            # 有3个或以上数据源，进行三源验证
            logger.info(f"   🔍 ETF {etf_code}: 发现数据冲突，启用第三数据源裁判机制")
            
            # 找出两个最接近的数据源（作为"多数派"）
            iopv_values = [(source_name, source_iopv) for source_name, source_iopv in sources]
            
            # 计算每对数据源的差异
            pairs_agreement = []
            for i in range(len(iopv_values)):
                for j in range(i + 1, len(iopv_values)):
                    name1, iopv1 = iopv_values[i]
                    name2, iopv2 = iopv_values[j]
                    diff = abs(iopv1 - iopv2) / max(iopv1, iopv2) * 100
                    pairs_agreement.append(((name1, iopv1), (name2, iopv2), diff))
            
            # 找出差异最小的一对（最可能正确）
            pairs_agreement.sort(key=lambda x: x[2])
            best_pair = pairs_agreement[0]
            (name1, iopv1), (name2, iopv2), min_diff = best_pair
            
            logger.info(f"   📊 数据源对比: {name1}(IOPV={iopv1:.4f}) vs {name2}(IOPV={iopv2:.4f}), 差异={min_diff:.1f}%")
            
            # 如果最接近的一对差异<10%，认为这两个数据源一致，采用它们的平均值
            if min_diff < 10:
                agreed_iopv = (iopv1 + iopv2) / 2
                agreed_rate = (primary_price - agreed_iopv) / agreed_iopv * 100
                
                # 检查是否与主数据源冲突
                primary_conflict = abs(agreed_iopv - primary_iopv) / primary_iopv * 100 > 20
                
                if primary_conflict:
                    logger.warning(f"   ⚠️ ETF {etf_code}: 主数据源({primary_source_type})与多数派冲突")
                    logger.info(f"   → 采用多数派IOPV: {primary_iopv:.4f} → {agreed_iopv:.4f}, 溢价率 {primary_rate:.2f}% → {agreed_rate:.2f}%")
                    df.loc[idx, "IOPV"] = agreed_iopv
                    df.loc[idx, "_temp_premium_rate"] = agreed_rate
                    corrected_count += 1
                else:
                    logger.info(f"   ✅ ETF {etf_code}: 主数据源与多数派一致，验证通过")
                    validated_count += 1
                
                extreme_mask[idx] = False
            else:
                # 三个数据源两两差异都很大，无法判定
                logger.warning(f"   ❓ ETF {etf_code}: 三个数据源两两差异都很大，无法判定（保留主数据源）")
                still_uncertain_count += 1
        
        elif has_large_diff and len(sources) == 2:
            # 只有2个数据源且差异大，无法判定，保留主数据源
            logger.warning(f"   ❓ ETF {etf_code}: 只有2个数据源且差异过大，无法判定（保留主数据源）")
            still_uncertain_count += 1
    
    # 统计验证结果
    still_extreme = extreme_mask.sum()
    logger.info(f"IOPV三源验证完成: 验证通过 {validated_count} 只, 修正 {corrected_count} 只, 存疑 {still_uncertain_count} 只")
    
    if still_extreme > 0 or still_uncertain_count > 0:
        # 仍有极端数据无法验证，但不删除——可能是真实的套利机会
        remaining_etfs = df.loc[extreme_mask, "ETF代码"].tolist()
        logger.warning(f"⚠️ {still_extreme + still_uncertain_count} 只ETF的IOPV无法通过交叉验证，但保留数据（可能是真实套利机会）: {remaining_etfs}")
...
def _fetch_all_iopv_sources(etf_codes: List[str], logger) -> Dict[str, Dict[str, float]]:
    ...
    for source_name, fetch_func in fetch_functions:
        try:
            ...
            logger.info(f"{source_name} 成功获取 {fetched_count} 个ETF的IOPV")

        except Exception as e:
            logger.error(f"从 {source_name} 获取IOPV失败: {str(e)}")
            continue

    logger.info(f"多数据源IOPV获取完成，共获取 {len(all_iopv_data)} 个ETF的数据")
