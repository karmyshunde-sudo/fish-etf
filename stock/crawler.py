...
def update_all_stocks_daily_data():
    ...
    for i, stock_code in enumerate(batch_codes):
        ...
        # 【关键修复】每处理MINOR_BATCH_SIZE只股票就检查一次提交状态
        if (i + 1) % MINOR_BATCH_SIZE == 0 and file_paths:
            ...
            try:
                ...
                subprocess.run(['git', 'add', DAILY_DIR], 
                             check=True, cwd=repo_root)
                logger.debug("✅ 已确保所有股票数据文件在暂存区")
            except Exception as e:
                logger.error(f"❌ 确保所有文件在暂存区失败: {str(e)}")
    ...
    if file_paths:
        ...
        try:
            ...
            subprocess.run(['git', 'add', DAILY_DIR], 
                         check=True, cwd=repo_root)
            logger.debug("✅ 已确保所有剩余股票数据文件在暂存区")
        except Exception as e:
            logger.error(f"❌ 确保剩余文件在暂存区失败: {str(e)}")
