...
def save_stock_daily_data(stock_code: str, df: pd.DataFrame):
    ...
    try:
        ...
        # 【关键修复】严格遵循ETF成功模式 - 简单有效的git add
        try:
            # 使用通配符添加所有股票数据文件
            cmd = f'git add "{DAILY_DIR}/*.csv"'
            logger.debug(f"执行Git命令: {cmd}")
            result = os.system(cmd)
            if result == 0:
                logger.debug(f"✅ 所有股票数据文件已添加到Git暂存区: {DAILY_DIR}/*.csv")
            else:
                logger.error(f"❌ 添加股票数据文件到Git暂存区失败，返回码: {result}")
        except Exception as e:
            logger.error(f"❌ 添加股票数据文件到Git暂存区失败: {str(e)}")
...
def update_all_stocks_daily_data():
    ...
    for i, stock_code in enumerate(batch_codes):
        ...
        if not df.empty:
            ...
            if (i + 1) % MINOR_BATCH_SIZE == 0 and file_paths:
                ...
                commit_success = commit_files_in_batches(file_paths, commit_msg)
    ...
    # 【关键修复】提交基础信息文件 - 按ETF成功模式
    commit_msg = f"更新股票基础信息 [skip ci] - {datetime.now().strftime('%Y%m%d%H%M%S')}"
    logger.info(f"提交基础信息文件: {commit_msg}")
    commit_success = commit_files_in_batches(BASIC_INFO_FILE, commit_msg)
