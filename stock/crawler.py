...
def create_or_update_basic_info():
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                # 【关键修复】使用更安全的请求方式
                stock_info = safe_get_stock_info()
                
                if stock_info.empty:
                    logger.warning(f"获取股票基础信息失败（尝试 {attempt+1}/{max_retries}），等待后重试...")
...
def safe_get_stock_info():
    """
    安全获取股票信息，包含反爬虫措施
    使用专业级请求头和随机延迟
    """
    try:
        # 【专业修复】使用更复杂的User-Agent，模拟真实浏览器
        user_agents = [
        ...
        # 【关键修复】使用akshare的内部请求机制，但添加自定义headers
        from akshare.utils.request import request
        ...
        # 尝试直接获取数据
        try:
            # 使用akshare的stock_zh_a_spot_em，但添加随机延迟
            time.sleep(random.uniform(1.5, 3.5))  # 模拟人类操作延迟
            
            # 尝试使用akshare的stock_zh_a_spot_em
            stock_info = ak.stock_zh_a_spot_em()
            return stock_info
        except Exception as e:
            logger.warning(f"直接使用ak.stock_zh_a_spot_em失败: {str(e)}，尝试备用方案")
            
            # 备用方案：直接请求数据接口
            data_url = "https://69.push2.eastmoney.com/api/qt/clist/get"
            params = {
                "pn": "1",
                "pz": "20000",
                "po": "1",
            ...
            # 发送请求
            response = request.get(data_url, params=params, headers=headers)
            response.raise_for_status()
            
            # 解析JSON数据
            json_data = response.json()
            
            if json_data and "data" in json_data and "diff" in json_data["data"]:
                ...
                logger.info(f"通过备用方案成功获取 {len(df)} 条股票基础信息")
                return df
        
        # 如果所有方法都失败，返回空DataFrame
        logger.error("所有获取股票信息的方法均已失败")
        return pd.DataFrame()
    
    except Exception as e:
        logger.error(f"安全获取股票信息失败: {str(e)}", exc_info=True)
        return pd.DataFrame()
def main():
    """主函数：更新所有股票数据"""
