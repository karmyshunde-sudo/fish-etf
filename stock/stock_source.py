...
def get_stock_daily_data_from_sources(stock_code: str, 
    ...
    try:
        ...
        # ===== 2. 定义完整的股票日线数据结构 =====
        # 这是股票日线数据的完整结构，必须严格遵循
        STOCK_DATA_STRUCTURE = [
            "日期", "股票代码", "开盘", "收盘", "最高", "最低", 
        ...
        for offset in range(total_priority):
            ...
            try:
                ...
                # 检查标准化后数据
                if not all(col in df.columns for col in STOCK_DATA_STRUCTURE):
                    missing = [col for col in STOCK_DATA_STRUCTURE if col not in df.columns]
        ...
        # ===== 6. 数据完整性保障 =====
        # 确保所有必要列存在
        for col in STOCK_DATA_STRUCTURE:
...
def _fetch_tencent_data(symbol: str, start_date: str, end_date: str, data_days: int, **kwargs) -> pd.DataFrame:
    """封装腾讯财经的API调用（直接URL）"""
    logger = logging.getLogger("StockCrawler")
...
def _fetch_sina_data(symbol: str, start_date: str, end_date: str, data_days: int, **kwargs) -> pd.DataFrame:
    """封装新浪财经的API调用（直接URL）"""
    logger = logging.getLogger("StockCrawler")
...
def _fetch_yfinance_data(symbol: str, start_date: str, end_date: str, data_days: int, **kwargs) -> pd.DataFrame:
...
def _standardize_data(df: pd.DataFrame, source_type: str, stock_code: str, logger) -> pd.DataFrame:
    ...
    if '股票代码' in df.columns:
        df['股票代码'] = df['股票代码'].apply(lambda x: format_stock_code_with_logger(str(x), logger))
    else:
        # 如果没有股票代码列，添加它
        df['股票代码'] = stock_code
...
def format_stock_code_with_logger(code, logger_instance):
    """
    规范化股票代码为6位字符串格式（带logger参数）
    Args:
    ...
    if not code_str.isdigit() or len(code_str) != 6:
        if logger_instance:
            logger_instance.warning(f"股票代码格式化失败: {code_str}")
        return None
...
def format_stock_code(code):
    """
    规范化股票代码为6位字符串格式（无logger参数，用于外部调用）
    Args:
        code: 股票代码（可能包含前缀或非6位）
    Returns:
        str: 规范化的6位股票代码
