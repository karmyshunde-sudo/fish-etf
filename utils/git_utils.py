...
def _verify_remote_file_exists(file_path):
    """
    验证文件是否真正存在于远程仓库
    Returns:
        bool: 验证是否通过
    ...
    try:
        ...
        if token and repo:
            ...
            if response.status_code == 200:
                logger.info("✅ 文件验证通过：文件存在于远程仓库 (API验证)")
                return True
            else:
        ...
        if result.returncode == 0 and result.stdout.strip():
            logger.info("✅ 文件验证通过：文件存在于远程仓库 (Git验证)")
            return True
        else:
...
def _commit_and_push(file_path, commit_message, max_retries=3):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # 验证文件是否真正存在于远程
            if file_path and os.path.exists(file_path):
                if _verify_remote_file_exists(file_path):
...
def force_commit_remaining_files():
    """
    強制提交所有剩余的文件
